...
src_install() {
	...
	mv bin/idea.properties bin/idea-${SLOT}.properties || die
	doins bin/idea-${SLOT}.properties || die
	rm bin/idea-${SLOT}.properties || die

	case $ARCH in
		amd64|ppc64)
			cat bin/idea64.vmoptions > bin/idea.vmoptions || die
			rm bin/idea64.vmoptions || die
			;;
	esac
	...
	ln -s /etc/idea/idea-${SLOT}.properties bin/idea.properties || die
	...
	fperms 755 "${dir}/bin/libdbm64.so" || die

	fperms -R 755 "${dir}"/jbr/bin/ || die
	fperms 755 "${dir}"/jbr/lib/{jexec,jspawnhelper} || die
