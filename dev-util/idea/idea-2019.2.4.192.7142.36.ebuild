...
src_prepare() {
	local ver=""
	einfo $(get_version_component_count)
	for index in $(seq $(get_version_component_count) -1 1); do
		local ver_tmp=$(get_version_component_range 1-${index})
		if [[ -f ${FILESDIR}/idea-${ver_tmp}.sh.patch ]]; then
			ver=${ver_tmp}
			break;
		fi
	done
	epatch ${FILESDIR}/idea-${ver}.sh.patch || die
}
