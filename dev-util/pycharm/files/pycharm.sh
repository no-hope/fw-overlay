...
if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/PyCharm2020.1/pycharm.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/PyCharm2020.1/pycharm.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/PyCharm2020.1/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/JetBrains/PyCharm2020.1/pycharm$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=PyCharm2020.1 \
